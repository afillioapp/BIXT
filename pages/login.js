...
export default function Login() {
  ...
  async function signInGoogle() {
    ...
    try {
      ...
      // router.replace called by onAuthStateChanged
    } catch (err) {
  ...
  return (
    <div className="login-wrap">
      ...
      <div className="login-brand">
        ...
        <h1 className="login-title">BIXT</h1>
        <p className="login-sub">
          Snap a receipt. AI reads it.
          <br />
          Saved straight to Drive.
        </p>
      </div>
      ...
      {step === "options" && (
        <div className="login-card">
          ...
          <button
            className="btn-auth btn-phone"
            onClick={() => { clearError(); setStep("phone"); }}
            disabled={!!loading}
          >
            <PhoneIcon />
            Continue with Phone
      ...
      {step === "phone" && (
        <div className="login-card">
          <button className="back-link" onClick={() => { clearError(); setStep("options"); }}>
            ← Back
          </button>
          <p className="otp-hint">Enter your phone number to receive a code.</p>
          <input
      ...
      {step === "otp" && (
        <div className="login-card">
          <button className="back-link" onClick={() => { clearError(); setStep("phone"); }}>
            ← Back
          </button>
      ...
      <p className="login-footer">
        By continuing you agree to BIXT's Terms &amp; Privacy Policy.
      </p>
    </div>
  );
}
