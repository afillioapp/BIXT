...
// Picks up the token Google appended to the URL when returning from the
// redirect flow. Strips it from the address bar immediately.
...
export function useDrive(user) {
  ...
  useEffect(() => {
    ...
    if (redirected) {
      gotTokenRef.current = true;
      setAccessToken(redirected.token);
      storeToken(redirected.token, redirected.expiresIn);
      scheduleSilentRefresh(redirected.expiresIn);
      loadRootAndProfile(redirected.token);
    ...
    if (stored) {
      ...
      scheduleSilentRefresh((stored.expiresAt - Date.now()) / 1000);
      loadRootAndProfile(stored.token);
      ensureClient().catch(() => {}); // ready for later renewals
      return () => {
        cancelled = true;
        clearTimeout(refreshTimerRef.current);
