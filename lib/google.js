...
// Reads all data rows (skips the header) from a sheet. Maps the 5 BIXT columns by position.
...
// Returns the ID of "BIXT - {companyName}" at the top of the user's Drive (creates it if missing).
...
export async function getCompanyRootFolderId(token, companyName) {
  return findOrCreateFolder(token, `BIXT - ${companyName}`, null);
}

// Locates the user's existing "BIXT - ..." root folder without knowing the
...
export async function findExistingCompanyRootFolder(token) {
  const q = encodeURIComponent(
    `mimeType='application/vnd.google-apps.folder' and trashed=false and 'root' in parents and name contains 'BIXT - '`
  );
  const list = await driveFetch(token, `${DRIVE_BASE}/files?q=${q}&fields=files(id,name)`);
  return list.files && list.files.length > 0 ? list.files[0].id : null;
}
