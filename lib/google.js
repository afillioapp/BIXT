...
async function ensureHeaderRow(token, spreadsheetId, headerRow) {
  ...
  const data = await driveFetch(token, `${SHEETS_BASE}/${spreadsheetId}/values/A1:${lastCol}1`);
  const firstRow = (data.values && data.values[0]) || [];
  if (firstRow[0] === headerRow[0] && firstRow.length >= headerRow.length) return;

  const gridId = await getFirstGridId(token, spreadsheetId);

  // Extend path: header exists (starts with "Date") but is missing trailing
  // columns (e.g. an older sheet from before Category existed) — write the
  // full header in place, no row insert, so existing data rows are untouched.
  if (firstRow[0] === headerRow[0] && firstRow.length < headerRow.length) {
    await driveFetch(
      token,
...
export async function listExpenseRows(token, spreadsheetId) {
  const data = await driveFetch(token, `${SHEETS_BASE}/${spreadsheetId}/values/A2:F`);
...
const EXPENSE_SHEET_HEADER = ["Date", "Place", "Total", "HST", "Receipt Link", "Category"];
