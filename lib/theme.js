...
//
// Also stamps the plain `dark` class on <html>: the ported Lovable design
// tokens (styles/tailwind.css, `@custom-variant dark (&:is(.dark *))`, from
// lovable-design/src/styles.css) key their own dark values off that class,
// not data-theme. Both attributes are set together so every rule from
// either system reacts to the same toggle.
